...
{
    m_pupilDetector.connectToCapture(m_capture);

    connect(&m_pupilDetector, &PupilDetectingFrameReceiver::pupilData,
            this, &CameraEyetracker::pupilData);

    m_pointCalibrationTimer.setSingleShot(true);
    connect(&m_pointCalibrationTimer, &QTimer::timeout, this, &CameraEyetracker::pointCalibrationTimeout);
    ...
    {
        m_hpeWindow = new HpeWidget(1, 0);

        connect(m_hpeWindow, &HpeWidget::hpeData, this, &CameraEyetracker::headData);
...
{
    if(m_cameraSetupDialog.isNull())
    {
        m_cameraSetupDialog = new PupilDetectorSetupDialog;
        connect(m_cameraSetupDialog, SIGNAL(finished(int)), this, SLOT(cameraSetupDialogFinished(int)));
        connect(m_cameraSetupDialog, SIGNAL(cameraIndexChanged(int)), this, SLOT(setCameraIndex(int)));
    }

    m_cameraSetupDialog->setVideoSource(&m_pupilDetector, m_cameraIndex);
    m_cameraSetupDialog->show();
}
...
void CameraEyetracker::headData(HpeData data, qint64 timestamp)
{
    Q_UNUSED(timestamp);

    m_headPos = cv::Point2d(data.translation[0], data.translation[1]);
    m_headRot = cv::Point3d(data.rotation[0], data.rotation[1], data.rotation[2]);
...
void CameraEyetracker::pupilData(bool ok,
                                 double posX,
                                 double posY,
                                 double size,
                                 qint64 timestamp)
{
    ...
    if(m_tracking)
    {
        ...
        emitNewPoint(gazePos, timestamp); // FIXME: timestamp must be in fractional seconds since epoch
    }
